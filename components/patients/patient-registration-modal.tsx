...
export function PatientRegistrationModal() {
  ...
  }, [isRegistrationModalOpen, fetchDoctorAvailability]);

  // Pre-select the most available doctor dynamically
  useEffect(() => {
    if (isRegistrationModalOpen && availableDoctors.length > 0 && !formData.doctorId) {
      // Since fetchDoctorAvailability sorts by least assigned, index 0 is the most available
      setFormData(prev => ({ ...prev, doctorId: availableDoctors[0].id }));
    }
  }, [isRegistrationModalOpen, availableDoctors, formData.doctorId]);
  ...
  return (
    <Modal 
      ...
      maxWidth="max-w-5xl"
    >
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8 font-sans w-full p-2">
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full justify-between pt-4 pr-1">
                    <div className="flex items-center gap-3">
                      {/* Avatar scales up slightly for the most available */}
